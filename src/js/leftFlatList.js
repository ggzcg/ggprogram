...
export default class LeftFlatList extends Component{
    ...
    renderRow =(item) =>{
        return(
            <TouchableOpacity key = {`${item.index}_${item}`} onPress={()=>this.cellAction(item)}>
                <View style={{height:60,flexDirection:'row',alignItems:'center'}}>
                    <View style={{height:50,width:5,backgroundColor: item.index == this.state.cell ? 'red' : 'rgba(0,0,0,0)'}}></View>
